...
+ (int)compareDate:(NSString*)startDate withDate:(NSString*)endDate formatter:(NSString *)formatterStr {
    if (!formatterStr.length) {
...
+ (int)compareDate:(NSString*)startDate withDate:(NSString*)endDate {
    ...
    return [self compareDate:startDate withDate:endDate formatter:@"yyyy-MM-dd HH:mm:ss"];
}

+ (NSString *)redPacketCompareCurrentTime:(NSString *)str {
    //把字符串转为NSdate
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
